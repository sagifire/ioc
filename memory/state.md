...
- `TASK-07.02-0057-stage-15-changesets-versioning-changelog` RUN-001 виконаний агентом і
  завершений після task-level human review approval.
- Stage 15 Changesets/versioning/changelog RUN-001 додав Changesets tooling,
  `.changeset/` config, synchronized fixed-version group для `@sagifire/ioc`,
  `@sagifire/ioc-next` and `@sagifire/ioc-testing`, root scripts, package changelog
  placeholders and a no-release empty changeset for setup changes.
